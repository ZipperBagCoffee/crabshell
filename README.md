...
- **Claude Code** uses `.claude-plugin/plugin.json`, `hooks/hooks.json`, `skills/` (every slash command, such as `/crabshell:save-memory`), and `commands/install-codex.md` (the legacy Codex bridge).
...
| `/crabshell:hotfix "description"` | One-pass work: a discussion whose Plan you confirm, one ticket, then the change |
