...
- **Why.** The user asked for "디스커션(플랜) -티켓" (the discussion carries the plan). v21.130.0 (`99473cd`) dropped the plan document but gave the discussion no place for it; the rules ("one-pass work → a discussion with one ticket (record after doing)"), the regressing description ("do the work directly and record it with hotfix") and the hotfix description ("Record directly-performed one-pass work"), all from v21.112.0 (`f931284`), told Claude to build first and record later, and nothing in the per-prompt rules mentioned the document skills. After building, nothing compared the result with the discussion. The commit gate counted a check only when typed exactly as declared, so `rtk npm test`, `cd app && npm test` or `npm test > out.txt` never unlocked it.
- **Plan in the discussion.** The discussing template has a `## Plan`: **Approach**, **Changes (file → what)**, **Order**, **Rejected alternatives**, **Risks**, **Analysis**, **Intent Check**, **User confirmation**. It may be revised until the first ticket exists; later changes are `Plan revision` log entries. Regressing cycle plans (`Cycle {n} plan` log entries) use the same fields. Creating a discussion ends by asking for the Plan and the user's confirmation before tickets.
- **Tickets carry the specifics.** The ticket template has `## Implementation Details` (this ticket's part of the plan: file → function/section → what changes) and a `## Intent Fidelity` result section: one row per Intent Anchor item and Plan decision — result, deviation (none/partial/departed), evidence, reason or user approval. A departure without the user's approval keeps the ticket from verified.
- **Blocked in code.** The docs guard blocks a new `D###_T###` ticket whose discussion has no plan (a filled `## Plan`, or a `Plan` / `Plan revision` / `Cycle N plan` log entry) or whose Implementation Details is empty or TBD — also with the ticketing skill active (`scripts/core/plan-entry.js`). The Codex document tool refuses the same (`--how` fills a discussion's Plan, `--details` is required for a discussion ticket) and writes both new sections. The log guard blocks `verified` while Intent Fidelity still holds its placeholder; `done` and older tickets without the section are unaffected.
- **Results against the discussion.** Regressing cycle verification (`references/cycle-verification.md`) compares each result with D's Intent Anchor and the cycle plan, not only the ticket's own checks, and carries every partial or departed row into the next cycle and the Final Report; Step 4d's quality check requires them. The verifying skill's run mode maps results onto the discussion's IA items and names items with no evidence.
- **Plan first, in the rules.** The RULES Workflows line says work that changes files starts in a discussion whose Plan the user confirms, tickets carry its specifics, and the result is compared with the discussion; the per-prompt checklist names the discussing and ticketing skills. The planning, hotfix and regressing skills (both hosts) and `/crabshell:hotfix`'s description say the same.
- **Commit gate.** A declared check also counts with one `cd <dir> &&` before it (the folder must be where the check is declared), output redirection after it (`> file`, `>> file`, `2>&1`, `&> file`), or the `rtk` wrapper; package.json `test` also counts as `pnpm test`, `pnpm run test`, `yarn test`, `yarn run test` and `bun run test` (`bun test` runs Bun's own test runner, not the script); a manifest with its runner has `node .crabshell/verification/run-verify.js` as its full check even without `tools` (`--changed` does not count as the full check). Pipes, `;` and `||` are still not accepted — the exit status would come from another command — and the block reason now names the accepted commands and forms.
- **Tests:** `_test-plan-first-docs.js`, `_test-plan-first-guards.js`, `_test-gate-loosening.js`; `_test-rule-wording.js` W11–W13; `_test-d-parent-tickets.js` C1 and `_test-codex-docs.js` updated for the plan requirement (contract change).
...
- Session memory is per session. The L1 read position, the save-interval counter (`memory/session-state/<first 8 characters of session_id>/`) and the delta watermark (`memory-index.json` `sessionDelta`) belong to one session, so two sessions in one project no longer duplicate or drop each other's L1 entries, suppress each other's L1 update, or trigger each other's saves. Each queued delta block names its session. A session that ends records its full-transcript position, so `--resume` continues without duplicates. Sessions that started before this release fall back to the last committed project-wide watermark once. Payloads without a session id keep the previous project-wide behavior.
- Incremental L1 refinement no longer drops the first new transcript line at a line boundary (this lost one line per save even in a single session) and holds back a trailing line that is still being written; offsets are byte-exact.
- File locks carry an owner token (`pid:token`), wait briefly for a live holder, take over only a dead or stale owner's lock — one process at a time — and release only their own lock. `core/state-lock.js` uses the same lock. Lock statistics count failed attempts as `skipCount`; `contendedCount` counts only real contention. A stale lock that cannot be removed ends in a timely skip instead of spinning.
- The commit gate is scoped to the shared working tree: another session's Read no longer clears an unverified edit. Prose, stylesheets and images do not arm the gate or invalidate a passing check; code, configuration and unknown files do. A project with no check configuration gets advice (Claude and Codex both pass it to the model) instead of a permanent block; a configuration with no runnable single command blocks with its own reason. A byte-order mark in the manifest is accepted. A check launched in the background (`backgroundTaskId`) is not a passing check. An interrupt invalidates only the interrupted session's checks. Reads no longer rewrite the verification state.
- Completion control keeps one entry per session in `completion-control.json` (schemaVersion 2; the previous flat file is read as its owner's entry). Another session's prompt no longer erases a pending child claim; payloads without a session id use their own entry. The SessionStart recovery record shows only the current session's record. A regressing run records its owning session (the session that runs its next skill takes over after `/clear` or a relaunch); another session's run does not make this session continue, while this session's own worklogs still do. Background-task notifications are not treated as the user's request.
- The document skill flag is per session with no 15-minute expiry; it is cleared when the session compacts or ends, so long skills can keep writing their own documents while another session cannot borrow the flag.
- SessionStart memory is assembled within a 9,500-character budget by priority (overview, active workflow, own recovery record, newest memory first, then summaries and the document digest), below Claude Code's 10,000-character inline limit (larger output reaches the model only as a 2,000-character preview).
- Tests: `_test-multisession-memory.js` (20), `_test-session-isolation.js` (30), `_test-session-start-budget.js` (9) with a shared `scripts/testlib/hook-harness.js`; each independent-review defect has a regression case that fails on the pre-fix code. Four existing tests no longer write the live repository state (`_test-fail-open-edge-cases.js` runs against a temporary copy of `scripts/`). Tests whose expectations changed with the contract: `_test-counter.js`, `_test-memory-rotation.js`, `_test-session-end-intent.js`, `_test-recovery-checkpoint.js`.
- Remaining limits: parallel hooks of the same session (parallel tools, subagents share the parent session id) can lose a counter increment, which only delays a save; session ids shorter than 8 characters use the project-wide mode; if SessionEnd cannot get the memory lock within 800 ms that session's last entries wait for no later extraction (its position is still recorded, so no duplicates); an older plugin version writing `completion-control.json` concurrently replaces the per-session entries; a subagent's own check (`agent_id`) still counts as parent evidence; `skills/status` reads only the legacy skill flag; Claude Stop continuation rarely engages because the last user-type transcript line is usually a tool result; `_v013-cycle1-check.js` still fails; 30 of 51 manifest entries need gitignored wiring files and fail on a fresh clone; `moc-digest.md` is stale.
